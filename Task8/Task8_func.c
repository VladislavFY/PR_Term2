...
int copy_database(StudentData *from, StudentData *to) {
 ...
 to->students = (Student *)malloc(from->count * sizeof(Student));
 if (to->students == NULL) {
  return -2;
 }
 to->count = from->count;
...
int generate_students(const char *surname_file_name, StudentData *database, int count, int min_group, int max_group, int min_school, int max_school) {
 ...
 database->students = (Student *)malloc(count * sizeof(Student));
 if (database->students == NULL) {
  return -6;
 }
...
int read_students(FILE *f, StudentData *database) {
 ...
 database->count = 0;
 database->students = (Student *)malloc(capacity * sizeof(Student));
 if (database->students == NULL) {
  return -2;
 }
 ...
 while (fscanf(f, "%127s %d %d", temp.Name, &temp.Group, &temp.School) == 3) {
  if (database->count >= capacity) {
   capacity *= 2;
   new_students = (Student *)realloc(database->students, capacity * sizeof(Student));
...
int compare_students(const void *a, const void *b) {
 ...
 if (student_a->School != student_b->School) {
  return student_a->School - student_b->School;
 }
...
void sort_students(StudentData *database) {
 ...
 qsort(database->students, database->count, sizeof(Student), compare_students);
}
...
int has_schoolmate_in_group(StudentData *database, int index, int group) {
 ...
 for (i = 0; i < database->count; ++i) {
  ...
  if (database->students[i].Group == group && database->students[i].School == database->students[index].School) {
   return 1;
  }
 }
 return 0;
...
void redistribute_students(StudentData *database) {
 ...
 for (i = 0; i < database->count; ++i) {
  int group_a = database->students[i].Group;
  int school_a = database->students[i].School;
...
int main_function(void) {
 ...
 if (choice == 2) {
  fprintf(f_out, "Original random list of students:\n");
  write_students(f_out, &original_database);
  fprintf(f_out, "\nProblem cases before redistribute: %d\n\n", problems_before);
  fprintf(f_out, "Changed list of students:\n");
  write_students(f_out, &database);
  fprintf(f_out, "\nProblem cases after redistribute: %d\n", problems_after);
 } else {
  write_students(f_out, &database);
 }
